...
theme.blogMapData = function(data, newurl) {
  var currentList = $("#AjaxinateContainer .m-recipes-list");
  var blogList = $(data).find("#AjaxinateContainer .m-recipes-list");
  currentList.replaceWith(blogList);
  $("#AjaxinatePagination").replaceWith($(data).find("#AjaxinatePagination"));
};

theme.blogpageMapData = function(data, newurl) {
  if( $(data).length ){
    $("#AjaxinateContainer .m-recipes-list").append($(data).find("#AjaxinateContainer .m-recipes-list").html());
    $("#AjaxinatePagination a").attr("disabled",false);
  } else{
    $("#AjaxinatePagination a").attr("disabled",true);
  }
  $("#AjaxinatePagination").replaceWith($(data).find("#AjaxinatePagination"));
};

theme.blogAjaxClick = function(baseLink) {
  theme.blogGetContent(baseLink);
  window.history.pushState("", baseLink, baseLink);
}

theme.blogpageAjaxClick = function(baseLink) {
  theme.blogpageGetContent(baseLink);
}

theme.blogGetContent = function(newurl) {
  $.ajax({
    type: 'get',
    url: newurl,
    success: function(data) {
      theme.blogMapData(data, newurl);
      theme.blogPaginationEvent();
      /*theme.blogtagEvent();*/
      $('#openDiv').removeClass( 's-is-active' );
    },
...
theme.blogpageGetContent = function(newurl) {
  $.ajax({
    type: 'get',
    url: newurl,
    success: function(data) {
      theme.blogpageMapData(data, newurl);
      theme.blogPaginationEvent();
      /*theme.blogtagEvent();*/
    },
    error: function(xhr, text) {
...
theme.blogFilterEvent = function(){
  $('.m-filter-option button').on('click', function () {
    var blogtagged = '';
    $( this ).toggleClass( 's-is-active' );
    $('.m-filter-option button').filter('.s-is-active').each(function(i) {
      if(blogtagged == ''){
        blogtagged = $(this).data('tag');
      }
      else{
        blogtagged = blogtagged + '+' + $(this).data('tag');
      }
    });
    if(blogtagged != ''){
      var blogurl = 'https://millicanpecan.com/blogs/pecan-recipes/tagged/' + blogtagged;
    }
    else{
      var blogurl = 'https://millicanpecan.com/blogs/pecan-recipes/';
    }
    /*window.location = blogurl;*/
    theme.blogAjaxClick(blogurl);
  });
};

theme.blogtagEvent = function(){
  $('.m-recipe-category a').on('click', function (e) {
    e.preventDefault();
...
theme.blogPaginationEvent = function(){
  $('#AjaxinatePagination a').on('click', function (e) {
    e.preventDefault();
    $( this ).attr("disabled",true);
    var pageurl = $( this ).attr('href');
    theme.blogpageAjaxClick(pageurl);
  });
};

theme.initRelateRecipeSlider = function() {
  var storiesSwiperGallery = new Swiper('.js-stories-gallery', {
    autoplay: 10 * 1000,
    loop: true,
    prevButton: '.js-stories-gallery-prev',
    nextButton: '.js-stories-gallery-next',
    centeredSlides: true,
    slidesPerView: 3,
    spaceBetween: 30,
    breakpoints: {
  ...
  return storiesSwiperGallery;
};

theme.initBlogToggle = function(){
  $('.m-recipe-finder__title').on('click', function (e) {
    e.preventDefault();
    $('#myDIV').toggleClass( 's-expanded' );
  });
                                  
  $('.m-filter__switcher').on('click', function (e) {
    e.preventDefault();
    $('#openDiv').toggleClass( 's-is-active' );
  });
};
...
theme.init = function() {
  ...
  theme.initBlogToggle();
  theme.blogPaginationEvent();
  /*theme.blogtagEvent();*/
  theme.blogFilterEvent();

  theme.initRelateRecipeSlider();
